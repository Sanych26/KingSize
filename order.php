<?php
    ...
    $sizeProduct = $connection-> query("SELECT * FROM `sizeProduct` WHERE product_id = $prodId");
    $products = $connection->query("SELECT * FROM `products` WHERE id = '$prodId'");
        ...
        <div class="registration">
            <div class="registration-left">
                ...
                <form class="registration-form" action="send/order_send.php" method="post">
                    <p class="registration-name registration-product-name"><?=$productName?></p>
                    <div class="registration-input-cont">
                        <div id="size">
                            <p class="frm">Розмір</p>
                            <img src="images/size.png" alt="">
                                <select name="size" >
                                    <?php
                                        while($size = $sizeProduct->fetch_assoc()) {
                                            if($size['size'] == $_SESSION['size']) {
                                                echo "<option selected='selected' value='".$size['size']."'>".$size['size']."</option>";
                                            }
                                            else {
